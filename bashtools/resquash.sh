...
if [ "${fix_participants}" -eq 1 ]; then
  ...
  singularity exec --overlay ${squash_file}:ro -B ${ST}/${part_dir}:/.st_tmp/ cp ${part_path} /.st_tmp/
  # remove subjects
  while read -r exc; do
...
if [ "${fix_participants}" -eq 1 ] || [ -n "${dataset_description}" ]; then
  squash_command="mksquashfs /to_squash/${part_dir_top} ${ST}/txtfix.squashfs -no-progress -noI -noD -noX -keep-as-directory -all-root -processors 1 -no-duplicates -fstime 1601265600"
...
if [ -f "${ST}/txtfix.squashfs" ]; then
  overlay="--overlay ${squash_file}:ro --overlay ${ST}/txtfix.squashfs"
else
...
singularity exec ${overlay} -B ${exclude_dir}:/.exclude/ -B ${output_dir}:/.out/ ${singularity_image} ${squash_command}
